...
@implementation MusicItemParser
...

{
    ...
    for (NSDictionary *perBook in books) {
        ...
        MusicItem *oneBook = [[MusicItem alloc] init];
...
- (void) parseFromUrl:(NSString *)url
{
    ...
    if (nHttpDownload) {
        ...
        nHttpDownload = [[MusicItemDownload alloc] initWithType:nDownloadType parseType:nParseType];
...
//            }
        ...
        NSLog(@"item %@ ...Item.cast..%@...type...%@....dur>>>>%@",item,item.pubDate,item.titles,item.summary);
...
- (void) musicDownloadComplete:(MusicItemDownload *)download
{
